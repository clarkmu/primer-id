...
commander
  .description(
    "Read a Newick tree, partition leaves into IN/OUT groups and compute the closest OUT neighbor for each leaf in the IN"
  )
  .version("0.0.1", "-v, --version")
  .option("-n --newick <newick>", "Input newick file")
  .option(
...
var csvStream = csv
  ...
  .on("end", function () {
    fs.readFile(commander.newick, (err, newick_data) => {
      ...
      var partitions = _.flatten([commander.regex].concat(commander.args)).map(
        (regex_str) => {
      ...
      tree.traverse_and_compute(function (node) {
        ...
        if (match != 1 && !node.data.children) {
          if (
            commander.default_compartment &&
            default_compartment_idx >= 0 &&
            ...
            group_by_index[node.cot_leaf_index] = [
              default_compartment_idx,
              node,
            ];
            partitions[default_compartment_idx].node_indices[
              node.cot_leaf_index
            ] = 1;
      ...
      const mean_and_variance = (s) => {
        ...
        const variance = _.max([
          _.reduce(s, (i, n) => i + (n[0] - mean) * (n[0] - mean) * n[1], 0),
          1 / count,
        ]);
      ...
      var csvStream = csv
        ...
        .transform(function (row) {
          return {
            ...
            partistic_tree_confidence:
              row.partistic && row.partistic["mode"] in partistic_support
                ? partistic_support[row.partistic["mode"]][1] /
                  partistic_support[row.partistic["mode"]][0]
                : "N/A",
            partistic_mean: row.partistic ? row.partistic["mean"] : "N/A",
            partistic_variance: row.partistic
              ? row.partistic["variance"]
              : "N/A",
            clade: row.clade ? row.clade["mode"] : "N/A",
            clade_support: row.clade ? row.clade["support"] : "N/A",
            clade_tree_cofidence:
              row.clade && row.clade["mode"] in clade_support
                ? clade_support[row.clade["mode"]][1] /
                  clade_support[row.clade["mode"]][0]
                : "N/A",
            ...
            placement_variance: row.placement
              ? row.placement["variance"]
              : "N/A",
      ...
      tree.traverse_and_compute(function (node) {
        if (tree.is_leafnode(node)) {
          var neighbor_nodes = _.map(node.cot_path_to_leaves_above, (v, k) => [
            v + node.cot_computed_length,
            +k,
          ])
            .filter(function (v, k) {
          ...
          neighbor_nodes = _.filter(
            neighbor_nodes,
            (v) => v[0] <= 2 * min_distance
          );

          let nn_count = neighbor_nodes.length;

          neighbor_nodes = _.countBy(
            neighbor_nodes.map((v) => [
              v[0],
              group_by_index[v[1]][1].decimal_date_value,
            ]),
            (v) => v[1]
          );
          ...
          while (
            ...
            +supported_parent.data.bootstrap_values < bs_cut
          ) {
            supported_parent = supported_parent.parent;
          }
          var spp = _.map(
            _.filter(
              supported_parent.descendants(),
              (n) => tree.is_leafnode(n) && n.data.trait != partitions[0].name
            ),
            (n) => n.decimal_date_value
          );
          while (
            supported_parent.parent &&
            _.size(spp) == 0 &&
          ...
          if (node.decimal_date_value) {
            ...
            partistic_support[node.decimal_date_value][1] +=
              summarize_set(neighbor_nodes)["mode"] == node.decimal_date_value;
            ...
            if (supported_parent.parent) {
              ...
              clade_support[node.decimal_date_value][1] +=
                summarize_set(_.countBy(spp))["mode"] ==
                node.decimal_date_value;
            }
          }
          ...
          if (node.data.trait == partitions[0].name) {
            ...
            qvoa_reports.push({
              ...
              clade: summarize_set(
                supported_parent.parent ? _.countBy(spp) : null
              ),
