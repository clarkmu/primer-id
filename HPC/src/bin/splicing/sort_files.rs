...
pub fn sort_files(dir: &str, destination: &str) -> Result<HashMap<String, RFiles>> {
    ...
    for file in files {
        ...
        if is_r1 {
            lib_name = file_name.to_lowercase().split("_r1").next().unwrap_or("").to_string();
        } else if is_r2 {
            lib_name = file_name.to_lowercase().split("_r2").next().unwrap_or("").to_string();
        }
        ...
        if is_r1 {
            results.entry(lib_name.clone()).or_insert_with(|| RFiles { r1: None, r2: None }).r1 =
                Some(destination.display().to_string());
        } else if is_r2 {
            results.entry(lib_name.clone()).or_insert_with(|| RFiles { r1: None, r2: None }).r2 =
                Some(destination.display().to_string());
        }
    }
